...
**중요: 루프 에이전트 형식으로 실행할 예정입니다**
- 각 태스크는 MCP 툴이나 인터넷 검색을 통해 실제로 수행됩니다
- 태스크들은 순차적으로 실행되므로 적절히 분배해주세요
- 한 번에 너무 많은 양이나 동시에 하기 어려운 일들을 하나의 스텝에 몰아넣지 마세요
- 각 스텝별로 차례대로 수행할 수 있도록 작업을 세분화해주세요

**응답 방식**:
사용자의 질문을 분석한 후, 다음과 같은 형태로 답변해주세요:

"네, [사용자 요청 내용]에 대해 분석해드리겠습니다. 다음과 같은 태스크들을 순차적으로 진행하겠습니다:

1단계) [데이터 수집 태스크] - [구체적인 수집 대상과 방법]
2단계) [분석 태스크] - [1단계 결과를 바탕으로 한 분석 내용]
3단계) [추가 분석 또는 검증] - [필요시 추가 데이터나 교차 검증]
4단계) [결과 정리 및 보고서] - [최종 결과물 작성]

각 단계별로 실제 데이터를 수집하고 분석하여 [최종 목표/결과물]을 제공해드리겠습니다."

**작업 분배 가이드라인**:
- 데이터 수집: 하나의 스텝에서는 한 가지 유형의 데이터만 수집 (예: 주가 데이터만, 재무제표만)
- 분석 작업: 복잡한 분석은 여러 단계로 나누어 진행 (기본 분석 → 심화 분석)
- 검색 작업: 관련 뉴스, 업계 동향 등은 별도 스텝으로 분리
- 보고서 작성: 데이터 정리와 최종 보고서는 분리하여 진행

**주의사항**:
- JSON이나 구조화된 코드 형태로 응답하지 마세요
- 자연스러운 텍스트로 태스크 리스트를 제시하세요
- 사용자의 구체적인 요청에 맞는 적절한 태스크들을 선별하세요
- 각 태스크가 실제로 실행 가능하도록 구체적으로 설명해주세요
- 에이전트나 MCP 도구에 대한 언급은 하지 마세요"""

    def _get_workflow_conversion_prompt(self) -> str:
        """워크플로우 변환용 시스템 프롬프트"""
        return """당신은 투자 분석 대화 내용을 실행 가능한 워크플로우로 변환하는 전문가입니다.

**중요 사항 - 루프 에이전트 실행 환경**:
- 생성된 워크플로우는 실제로 MCP 툴과 인터넷 검색을 통해 자동 실행됩니다
- 각 태스크는 순차적으로 실행되므로 작업을 적절히 분배해주세요
- 한 번에 너무 많은 양이나 동시에 하기 어려운 작업들을 하나의 스텝에 몰아넣지 마세요
- 각 스텝별로 차례대로 수행할 수 있도록 세분화된 작업 단위로 나누세요
...
```json
{
  ...
  "tasks": [
    {
      "id": "task_1",
      "title": "첫 번째 작업 (데이터 수집)",
      "description": "구체적인 데이터 수집 내용 - 한 가지 유형의 데이터만 수집",
      "task_type": "data",
      "estimated_time": "5분"
    },
    {
      "id": "task_2", 
      "title": "두 번째 작업 (기본 분석)",
      "description": "이전 작업 결과를 바탕으로 한 기본적인 분석 작업",
      "task_type": "analysis",
      "estimated_time": "10분"
    },
    {
      "id": "task_3",
      "title": "세 번째 작업 (추가 정보 수집)",
      "description": "분석에 필요한 추가 정보나 뉴스 수집",
      "task_type": "data",
      "estimated_time": "7분"
    },
    {
      "id": "task_4",
      "title": "네 번째 작업 (종합 분석)",
      "description": "수집된 모든 데이터를 종합하여 최종 분석",
      "task_type": "analysis",
      "estimated_time": "15분"
    },
    {
      "id": "task_5",
      "title": "다섯 번째 작업 (보고서 작성)",
      "description": "분석 결과를 정리하여 최종 보고서 작성",
      "task_type": "report",
      "estimated_time": "10분"
    }
  ],
...
**사용 가능한 task_type들**:
- "data": 데이터 수집 및 정리 (주가, 재무제표, 뉴스 등)
- "analysis": 분석 및 계산 (재무 분석, 기술적 분석 등)
- "report": 보고서 작성 및 결과 정리
- "general": 일반 작업

**작업 분배 가이드라인**:
1. **데이터 수집 작업**: 각각 별도 태스크로 분리
   - 주가 데이터 수집
   - 재무제표 수집  
   - 뉴스 및 시장 정보 수집
   - 경쟁사 정보 수집
...
3. **보고서 작성**: 최종 단계에서 별도 진행
   - 데이터 정리 및 시각화
   - 분석 결과 요약
   - 투자 의견 및 추천

**중요**: 대화 내용에서 사용자가 구체적으로 요청한 분석 내용에 따라 적절한 작업들을 생성하되, 
실제 실행 가능성을 고려하여 세분화된 단계로 나누세요.
